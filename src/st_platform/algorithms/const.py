...
class ConSTDomainAlgorithm(Algorithm):
    spec = AlgorithmSpec(
        algorithm_id="const",
        name="conST Domain Detection",
        task_type=TaskType.DOMAIN_DETECTION,
        runtime="python",
        version="benchmarkst",
        description=(
            "Runs the local conST model encoder on a spatial KNN graph and clusters "
            "the learned representations into domains."
        ),
    ...
    def run(
        ...
        adjacency = _normalized_dense_adjacency(
            xy=xy,
        ...
        model_params = SimpleNamespace(
            feat_hidden1=int(parameters.get("feat_hidden1", 8)),
            feat_hidden2=int(parameters.get("feat_hidden2", 4)),
            gcn_hidden1=int(parameters.get("gcn_hidden1", 4)),
            gcn_hidden2=int(parameters.get("gcn_hidden2", 2)),
            p_drop=float(parameters.get("dropout", 0.0)),
            dec_cluster_n=n_clusters,
        )
        features_tensor = backend.torch.tensor(features, dtype=backend.torch.float32)
        model = backend.conST(
        ...
        optimizer = backend.torch.optim.Adam(
            model.parameters(),
            lr=learning_rate,
        ...
        with warnings.catch_warnings(), contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(
            ...
            for _ in range(max(0, epochs)):
                optimizer.zero_grad()
                z, mu, logvar, decoded, q, feat_x, gnn_z = model(features_tensor, adjacency)
                loss = backend.functional.mse_loss(decoded, features_tensor)
                loss.backward()
                optimizer.step()
            model.eval()
            with backend.torch.no_grad():
                embedding, _, _, _, _, _, _ = model(features_tensor, adjacency)
        ...
        output_path = Path(str(context["run_root"])) / "const-domains.json"
        ...
        return AlgorithmOutput(
            summary={
                "message": "conST domain detection completed.",
                "backend": "conST",
            ...
            artifacts=[
                {
                    ...
                    "uri": str(output_path),
                    "description": "Per-spot conST spatial domain assignments.",
                }
            ],
            metrics={
...
def _load_backend() -> _Backend:
    ...
    const_root = repo_root / "BenchmarkST" / "conST"
    if str(const_root) not in sys.path:
        sys.path.insert(0, str(const_root))
    try:
        ...
        raise RuntimeError(
            "conST dependencies are not installed. Install torch and scikit-learn into the uv environment."
        ) from exc
    return _Backend(
        KMeans=KMeans,
        conST=conST,
...
def _normalized_dense_adjacency(
    xy: np.ndarray,
    neighbor_k: int,
    kneighbors_graph: Any,
    torch_module: Any,
