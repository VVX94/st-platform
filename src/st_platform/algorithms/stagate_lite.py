...
class STAGATELiteDomainAlgorithm(Algorithm):
    spec = AlgorithmSpec(
        algorithm_id="stagate-lite",
        name="STAGATE Lite Domain Detection",
        task_type=TaskType.DOMAIN_DETECTION,
        runtime="python",
        version="0.1.0",
        description=(
            "A dependency-light STAGATE adapter using PCA embeddings with "
            "spatial neighborhood refinement for domain detection."
        ),
        tags=("classic", "stagate", "domain", "lightweight"),
    )
    ...
    def run(
        ...
        n_components = int(parameters.get("n_components", min(50, n_spots - 1, n_genes)))
        neighbor_k = int(parameters.get("neighbor_k", 6))
        neighbor_k = max(1, min(neighbor_k, n_spots - 1))
        spatial_weight = float(parameters.get("spatial_weight", 0.15))
        ...
        output_path = Path(str(context["run_root"])) / "stagate-lite-domains.json"
        ...
        return AlgorithmOutput(
            summary={
                "message": "STAGATE-lite domain detection completed.",
            ...
            artifacts=[
                {
                    ...
                    "uri": str(output_path),
                    "description": "Per-spot STAGATE-lite spatial domain assignments.",
                }
            ],
            metrics={
